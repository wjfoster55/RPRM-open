...
def run_cases() -> list[dict]:
    ...
    def transported_receiver() -> dict:
        bijections = set()
        for perm in itertools.permutations(range(3)):
            for offset in range(8):
                bijections.add(tuple(sum(((s >> perm[j]) & 1) << j for j in range(3)) ^ offset
                                     for s in range(8)))
        require(len(bijections) == 48, 'Wrong cube symmetry count.')
        for m in bijections:
            retained_after = tuple(s for s in range(8) if m[s] in LOW)
            require(len(retained_after) == 7, 'Transport lost an address.')
            for siblings in pairs.values():
                f, g = siblings
                out_f, out_g = pullback(f, m), pullback(g, m)
                require(all(out_f[s] == out_g[s] for s in retained_after),
                        'Coherently transported observation became insufficient.')
                omitted_after = next(s for s in range(8) if m[s] == 7)
                require(out_f[omitted_after] != out_g[omitted_after],
                        'Transport erased the actual unresolved distinction.')
        return {'bijections': len(bijections), 'collision_pairs_per_bijection': len(pairs),
                'transported_receiver': 'S_new = map^(-1)(S_old)',
                'result': 'No information loss is created by coherent transport of both view and readout.',
                'contrast': 'CB06 keeps the original retained receiver after changing the input view.'}
    case('CB12_TRANSPORTED_RECEIVER', transported_receiver)
    return rows
