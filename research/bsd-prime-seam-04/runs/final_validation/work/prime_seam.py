...
def div(z,w):
    ...
    return (v[0]/n,v[1]/n)
def power(z,n):
    if n<0:return power(div(ONE,z),-n)
    out=ONE
    while n:
...
def oriented_five_valuation(z):
    """Exact valuation at i=2 mod5; certify precision using the integer norm."""
    denominator=z[0].denominator*z[1].denominator//gcd(z[0].denominator,z[1].denominator)
    a=int(z[0]*denominator);b=int(z[1]*denominator)
    modulus=5;root=2
    # Once modulus exceeds the nonzero integer norm, a zero evaluated residue
    # would contradict the valuation of the product with the conjugate.
    while modulus<=a*a+b*b:
        correction=(-(root*root+1)//modulus*pow(2*root,-1,5))%5
        root+=correction*modulus;modulus*=5
        assert (root*root+1)%modulus==0
    residue=(a+b*root)%modulus
    assert residue!=0
    v=0
    while residue%5==0:residue//=5;v+=1
    while denominator%5==0:denominator//=5;v-=1
    return v
...
def main():
    ...
    if not 37<=args.bound<=10000:raise ValueError('frozen scout admits bounds 37..10000')
    ...
    oriented_rows=[]
    joint_valuation_rows=[]
    for row in rows:
        p=row['prime'];v=oriented_five_valuation(local[p])
        if p!=5:
            pi=tuple(F(x) for x in row['pi'])
            a=oriented_five_valuation(sub(pi,ONE))
            b=oriented_five_valuation(sub(conj(pi),ONE))
            s=row['point_count_factors'].get('5',0)
            assert a>=0 and b>=0 and a+b==s and b-a==v
            assert (s-v)%2==0 and (s+v)%2==0
            assert (s-v)//2==a and (s+v)//2==b
            joint_valuation_rows.append({'prime':p,'alpha_minus_one_v5':a,
                'beta_minus_one_v5':b,'count_valuation_sum':s,
                'factor_valuation_difference':v,'joint_inverse':[a,b]})
        if p in same_5_support:
            stripped=mul(local[p],power(c5,v))
            assert oriented_five_valuation(stripped)==0
            assert stripped[0].denominator%5 and stripped[1].denominator%5
            oriented_rows.append({'prime':p,'lucky':p in lucky,'v5_Cp':v,
                                  'factor_after_multiplying_by_C5_power_v':enc(stripped)})
    assert oriented_five_valuation(c5)==-1
    assert oriented_five_valuation(c13)==-1
    assert oriented_five_valuation(local[29])==1
    assert mul(local[29],c5)==(F(21,29),F(-20,29))
    assert div(local[37],c5)==(F(35,37),F(12,37))
    report={
        ...
        'oriented_5_seams':oriented_rows,
        'joint_valuation_shadows':joint_valuation_rows,
