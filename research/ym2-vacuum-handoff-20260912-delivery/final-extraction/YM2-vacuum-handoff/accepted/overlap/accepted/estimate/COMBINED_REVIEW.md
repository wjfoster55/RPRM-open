...
- the bounded-tilt lemma used in `accepted/CONDITIONAL_REFINEMENT.md`, CR12; its original C4 proof was additionally read during the audit.
...
For a self-contained check of that normalization, let the unnormalized likelihood ratio h lie in [a,b], and let k be its expectation under the reference probability. Then TV=E[(h-k)_+]/k. The convex chord bound gives TV<=(b-k)(k-a)/[k(b-a)]. Maximizing over k in [a,b] gives (sqrt(b)-sqrt(a))/(sqrt(b)+sqrt(a))=tanh(log(b/a)/4). The case a=b has TV=0.
...
## 8. Final result-note cross-check

The final `RESULT.md` and `README.md` were checked against this review. Their combined interval, quantum normalization, three-dimensional specializations, evidence grade, and continuum qualifications agree with the proved chain. At d=3 and r=1/64, the smaller root is Y=3/8, so RV1 gives 3/(4e), as stated. The old strict endpoint also closes at gap>=1/4 by the same fixed-graph continuity argument; this is consistent with the separate inverse note and is superseded by RV2.

The enlarged half-gap interval also preserves the accepted CP8 head-and-remainder readout. With source parameter t=4mr, b=8/9, and Y=t+bY^2, the unchanged remainder proof gives W=Y-t-bt^2. At r=7/(144m), t=7/36 and Y=1/4, so W=16/729. The inherited head bound consequently gives exactly

```text
row <= 7/144 + 49(m-1)/(151632m) + 64/2187 < 1/12.
```

Exact rational arithmetic checked the endpoint value and the strict inequality separately for m=2 and m=4. The margins below 1/12 are respectively 4819/909792 and 9491/1819584. The scalar W is nondecreasing over this interval: using t=Y-bY^2 gives W=b^2 Y^3(2-bY), whose derivative is positive for 0<Y<=1/4. Thus the endpoint substitution bounds the whole interval, including the actual infinite Fourier remainder. This does not change the head coefficients or assert that a truncated vacuum is the actual density.

The final conditional note uses the quadratic-form notation q_(A-E)[Uf], which covers the claimed form domain. Section 6 of `RECENTER_AND_INVERSE.md` correctly confines the remaining OPEN statement to an improved inverse estimate within that route, while acknowledging the source and bilinear improvements established elsewhere in this continuation.
