...
def main():
    ...
    names=('coefficient.gp','anchor_control.gp','padic_height.py','rational_ec.py','readback.py','run_coefficient.py')
    ...
    jobs=[('analytic',[str(gp),'-q','-f','-s','256M','-D','parisizemax=1000000000',str(target/'work/coefficient.gp')]),
          ('anchor_control',[str(gp),'-q','-f','-s','256M',str(target/'work/anchor_control.gp')]),
    ...
    for label,command in jobs:
        ...
        sentinel={'analytic':'END_EXACT_COEFFICIENT_05','anchor_control':'END_ANCHOR_CONTROL'}.get(label)
        ok=p.returncode==0 and (sentinel is None or (sentinel in payload and '***' not in payload))
