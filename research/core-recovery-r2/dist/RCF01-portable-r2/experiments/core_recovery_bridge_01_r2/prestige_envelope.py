#!/usr/bin/env python3
"""Operation-aware Prestige envelope for RCF01-R2 guard repair.

Successor of frozen RCF01-envelope-1-r1 (original and R1 bytes retained in
experiments/core_recovery_bridge_01/ and core_recovery_bridge_01_r1/).
This copy keeps the R1 composition-recipe mechanism and repairs output-family
rules plus the generic-map admission/metadata boundary. It is not a
retroactive protocol rewrite and not a new recovery.
...
INHERITED_R1_ENVELOPE_VERSION = "RCF01-envelope-1-r1"
ENVELOPE_VERSION = "RCF01-envelope-1-r2"
...
def named_map_for_mapping(mapping: Sequence[int]) -> Optional[str]:
    mapping_t = tuple(int(x) for x in mapping)
    for name, candidate in input_maps().items():
        if candidate == mapping_t:
            return name
    return None


def _validate_site_map(mapping: Sequence[int]) -> Optional[Result]:
    if len(mapping) != N_SITES:
        return Result("NONE", "UNSUPPORTED", witness="Need an eight-site map")
    if not all(type(x) is int and 0 <= x < N_SITES for x in mapping):
        return Result("NONE", "UNSUPPORTED", witness="Map leaves the cube")
    return None
...
def promote_cube(table: Sequence, *, label: str, contract: FrozenSet[str],
    ...
    METER.constructions += 1
    full_table = frac_tuple(table)
    if family not in ("rational_cube", "boolean_cube"):
        raise AdmissionError("cube promotion family must be rational_cube or boolean_cube")
    if family == "boolean_cube" and any(v not in (Fraction(0), Fraction(1)) for v in full_table):
        raise AdmissionError("boolean_cube claim rejected: table is not {0,1}-valued")
    full = mobius(full_table)
    retained = full[:7]
    ...
    if cold_dir is not None:
        payload = {
            ...
            "full_table": [str(v) for v in full_table],
...
UNCHECKED_MAP_CONTRACT = frozenset({"read_admitted"})
COORDINATE_BITS = {"h": 0, "x": 1, "y": 2}
NAMED_COORDINATE_MAPS = frozenset({
    "identity",
    "clamp_h_0", "clamp_h_1", "clamp_x_0", "clamp_x_1", "clamp_y_0", "clamp_y_1",
    "flip_h", "flip_x", "flip_y",
})
...
def _family_after_add(a: Envelope, b: Envelope) -> str:
    """Addition is over Q. Conservative: never certify boolean_cube from a sum.

    Does not inspect B2 values and does not read the omitted site. A narrower
    Boolean output class would need a global nonoverlap guarantee this API
    does not compute. Mixed Boolean/rational cube operands are lifted here.
    """
    return "rational_cube"


def _family_after_mul(a: Envelope, b: Envelope) -> str:
    """Pointwise multiplication. {0,1} is closed, so Boolean×Boolean stays Boolean.

    Does not inspect B2 values to certify a global class. Mixed cube operands
    lift to rational_cube.
    """
...
def compatible(a: Envelope, b: Envelope) -> Optional[str]:
    cube = {"boolean_cube", "rational_cube"}
    if a.family != b.family and not (a.family in cube and b.family in cube):
        return "family"
...
def add_units(a: Envelope, b: Envelope) -> Result:
    ...
    return Result("ONE", "EXACT", value=out, witness={
        ...
        "output_family_rule": "q_addition_lifts_to_rational_cube",
        "mixed_cube_coercion": a.family != b.family,
    })
...
def multiply_units(a: Envelope, b: Envelope) -> Result:
    ...
    return Result("ONE", "EXACT", value=out, witness={
        ...
        "output_family_rule": ("boolean_multiply_stays_boolean"
                               if family == "boolean_cube" else "rational_multiply"),
        "mixed_cube_coercion": a.family != b.family,
    })
...
def apply_fixed_receiver_map(env: Envelope, mapping: Sequence[int]) -> Result:
    """Certified public generic map: named coordinate maps only.

    Same admission as apply_input_map. Arbitrary eight-site maps are not
    certified here; use apply_raw_site_map for an unchecked/reference helper.
    Coordinate flips/clamps keep the advertised degree bound because they are
    affine in the cube coordinates.
    """
    invalid = _validate_site_map(mapping)
    if invalid is not None:
        return invalid
    name = named_map_for_mapping(mapping)
    if name is None or name not in NAMED_COORDINATE_MAPS:
        return Result(
            "NONE", "UNSUPPORTED",
            witness="not a named coordinate map; use apply_raw_site_map for unchecked reference",
        )
    return apply_input_map(env, name)


def apply_raw_site_map(env: Envelope, mapping: Sequence[int]) -> Result:
    """Unchecked/reference helper. Not a certified public envelope.

    Validates the eight-site carrier. Returns values with degree_bound unset
    and contract reduced to read_admitted, so the result cannot be treated as
    a certified public continuation without the missing obligations.
    """
    METER.hot_ops += 1
    invalid = _validate_site_map(mapping)
    if invalid is not None:
    ...
    mapping_t = tuple(int(x) for x in mapping)
    spec = _operand_spec(env)
    mapped_recipe = ("pullback_sites", mapping_t, spec) if spec is not None else None
    if mapped_recipe is not None:
        METER.constructions += 1
    if map_closes_b2(mapping_t):
        pulled = tuple(eval_retained(env.retained, mapping_t[s]) for s in B2)
        retained = compact_from_b2(pulled)
        out = Envelope(
            family=env.family, visible_unit_label=env.visible_unit_label,
            retained=retained, contract=UNCHECKED_MAP_CONTRACT,
            carrier_id=env.carrier_id, frame_id=env.frame_id, version=env.version,
            cold_route=None, cold_sha256=None, history=env.history,
            recipe=mapped_recipe, degree_bound=None,
        )
        return Result("ONE", "EXACT", value=out, witness={
            "closed_on_B2": True,
            "role": "unchecked_reference",
            "certified": False,
            "degree_bound": None,
        })
    reopened = _reopen_cube_table(env)
    if reopened.status != "EXACT":
        return reopened
    table = pullback_table(reopened.value, mapping_t)
    out = Envelope(
        family=env.family, visible_unit_label=env.visible_unit_label,
        retained=compact_from_table(table), contract=UNCHECKED_MAP_CONTRACT,
        carrier_id=env.carrier_id, frame_id=env.frame_id, version=env.version,
        cold_route=None, cold_sha256=None, history=env.history,
        recipe=mapped_recipe, degree_bound=None,
    )
    return Result("ONE", "EXACT", value=out, witness={
        "closed_on_B2": False, "used_cold": True,
        "role": "unchecked_reference", "certified": False, "degree_bound": None,
    })
