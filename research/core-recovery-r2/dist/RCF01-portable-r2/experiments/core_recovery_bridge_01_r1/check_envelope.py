#!/usr/bin/env python3
"""RCF01 envelope checks. Successor protocol RCF01-envelope-1-r1.

Frozen original 22-case suite is preserved at
experiments/core_recovery_bridge_01/ (protocol RCF01-envelope-1).
This successor re-runs those IDs against the repaired API and adds PE23–PE26.
Does not import check_bridge.py. Standard library only besides adjacent rprm.
"""
...
def run_cases(work: Path) -> list[dict]:
    ...
    def composed_product_chain() -> dict:
        before = meter_snapshot()
        h = unit("h", contract=ARITH_CONTRACT, cold_dir=None, name="pe23_h")
        x = unit("x", contract=ARITH_CONTRACT, cold_dir=None, name="pe23_x")
        y = unit("y", contract=ARITH_CONTRACT, cold_dir=None, name="pe23_y")
        hx = multiply_units(h, x)
        require(hx.status == "EXACT", "h*x hot multiply failed")
        require(hx.value.degree_bound == 2, "h*x must leave degree <=1")
        p = multiply_units(hx.value, y)
        require(p.status == "EXACT", "(h*x)*y hot multiply failed")
        require(p.value.retained == tuple(Fraction(0) for _ in range(7)),
                "C2 of hxy must be the zero summary")
        require(p.value.degree_bound == 3, "(h*x)*y degree bound must be 3")
        after = meter_snapshot()
        require(after["hot_ops"] - before["hot_ops"] >= 2, "Multiplies must count as hot ops")
        require(after["cold_reads"] == before["cold_reads"],
                "Hot multiply must not read cold artifacts")
        require(p.value.recipe is None, "Hot-only units must not invent a composition recipe")
        padded_zero = zeta(tuple(p.value.retained) + (Fraction(0),))
        require(mobius(padded_zero)[7] == 0, "Zero-pad control")
        return {"chain": "multiply(multiply(unit(h), unit(x)), unit(y))",
                "hot_c2": [str(v) for v in p.value.retained],
                "degree_bounds": {"h": h.degree_bound, "hx": hx.value.degree_bound,
                                  "p": p.value.degree_bound},
                "affine_inherited_after_hx": False,
                "hot_ops_delta": after["hot_ops"] - before["hot_ops"],
                "cold_reads_delta": after["cold_reads"] - before["cold_reads"],
                "constructions_delta": after["constructions"] - before["constructions"],
                "zero_pad_is_not_the_composed_law": True}
    case("PE23_COMPOSED_PRODUCT_CHAIN", composed_product_chain)

    def composed_flip_branches() -> dict:
        h_a = unit("h", contract=ARITH_CONTRACT, cold_dir=None, name="pe24_ha")
        x_a = unit("x", contract=ARITH_CONTRACT, cold_dir=None, name="pe24_xa")
        y_a = unit("y", contract=ARITH_CONTRACT, cold_dir=None, name="pe24_ya")
        p_a = multiply_units(multiply_units(h_a, x_a).value, y_a).value
        outside = apply_input_map(p_a, "flip_h")
        require(outside.status == "UNSUPPORTED", "Arithmetic contract must refuse flip")

        h_b = unit("h", contract=FLIP_CONTRACT, cold_dir=None, name="pe24_hb")
        x_b = unit("x", contract=FLIP_CONTRACT, cold_dir=None, name="pe24_xb")
        y_b = unit("y", contract=FLIP_CONTRACT, cold_dir=None, name="pe24_yb")
        p_b = multiply_units(multiply_units(h_b, x_b).value, y_b).value
        admitted = apply_input_map(p_b, "flip_h")
        require(admitted.status == "REOPEN_REQUIRED" and admitted.disposition == "MANY",
                "Admitted flip without backing must reopen")
        require(admitted.value is None, "Must not fabricate a flipped table from the zero C2")

        h_c = unit("h", contract=FLIP_CONTRACT, cold_dir=cold, name="pe24_hc")
        x_c = unit("x", contract=FLIP_CONTRACT, cold_dir=cold, name="pe24_xc")
        y_c = unit("y", contract=FLIP_CONTRACT, cold_dir=cold, name="pe24_yc")
        p_c = multiply_units(multiply_units(h_c, x_c).value, y_c)
        require(p_c.value.recipe is not None, "Backed units must retain a composition recipe")
        repaired = apply_input_map(p_c.value, "flip_h")
        require(repaired.status == "EXACT", "Composed recipe must reopen the product, not an operand")
        require(eval_retained(repaired.value.retained, 6) == 1, "Flipped hxy at (0,1,1) must be 1")
        zero = promote_cube((0,) * 8, label="one", contract=FLIP_CONTRACT, cold_dir=cold, name="pe24_zero")
        zflip = apply_input_map(zero, "flip_h")
        require(zflip.status == "EXACT" and eval_retained(zflip.value.retained, 6) == 0,
                "Actual zero law still reads 0 after flip")
        hflip = apply_input_map(h_c, "flip_h")
        require(hflip.status == "EXACT", "Operand flip with cold must be exact")
        require(repaired.value.retained != hflip.value.retained,
                "Operand h flipped is not the composed product")
        require(eval_retained(hflip.value.retained, 0) == 1,
                "Flipped h at origin is 1; flipped hxy is 0")
        return {"arith_contract": outside.status,
                "admitted_no_backing": {"status": admitted.status, "disposition": admitted.disposition},
                "backed_product": {"status": repaired.status,
                                   "readout_011": str(eval_retained(repaired.value.retained, 6)),
                                   "zero_law_readout_011": "0",
                                   "used_composed_recipe": True}}
    case("PE24_COMPOSED_FLIP_BRANCHES", composed_flip_branches)

    def composed_corrupt_dependency() -> dict:
        h = unit("h", contract=FLIP_CONTRACT, cold_dir=cold, name="pe25_h")
        x = unit("x", contract=FLIP_CONTRACT, cold_dir=cold, name="pe25_x")
        y = unit("y", contract=FLIP_CONTRACT, cold_dir=cold, name="pe25_y")
        p = multiply_units(multiply_units(h, x).value, y).value
        path = Path(h.cold_route)
        original = path.read_bytes()
        path.write_bytes(original.replace(b"BOOL_CUBE_Q_n3", b"BOOL_CUBE_Q_n9", 1))
        corrupted = apply_input_map(p, "flip_h")
        require(corrupted.status == "INVALID_DEPENDENCY", "Mutated operand must invalidate the recipe")
        require(corrupted.value is None, "Must not emit an exact flipped summary after corruption")
        path.write_bytes(original)
        path.unlink()
        missing = apply_input_map(p, "flip_h")
        require(missing.status in ("UNRESOLVED", "INVALID_DEPENDENCY"),
                "Deleted operand backing was accepted")
        return {"corrupted_operand_status": corrupted.status,
                "deleted_operand_status": missing.status,
                "did_not_infer_zero_from_hot_c2": True}
    case("PE25_COMPOSED_CORRUPT_DEPENDENCY", composed_corrupt_dependency)

    def output_class_and_fibers() -> dict:
        h = unit("h", contract=ARITH_CONTRACT, cold_dir=None, name="pe26_h",
                 family="boolean_cube")
        doubled = add_units(h, h)
        require(doubled.status == "EXACT", "h+h should have an exact C2 summary")
        require(eval_retained(doubled.value.retained, 1) == 2, "h+h is 2h over Q")
        require(doubled.value.family == "rational_cube",
                "Boolean-valued class must not survive Q addition that leaves {0,1}")
        require(doubled.disposition == "ONE", "Unique C2 output is ONE, not a Boolean object fiber")

        zero = promote_cube((0,) * 8, label="one", contract=ARITH_CONTRACT,
                            cold_dir=None, name="pe26_z")
        spike = promote_cube((0,) * 7 + (1,), label="one", contract=ARITH_CONTRACT,
                             cold_dir=None, name="pe26_spike")
        require(zero.retained == spike.retained == tuple(Fraction(0) for _ in range(7)),
                "zero and hxy share C2")
        hot_sum = add_units(zero, spike)
        require(hot_sum.status == "EXACT" and hot_sum.disposition == "ONE",
                "API reports unique C2 output")
        require(hot_sum.value.recipe is None, "Hot-only add must not claim a unique full law")
        require(hot_sum.value.retained == tuple(Fraction(0) for _ in range(7)),
                "Shared C2 of independent sums")

        p = multiply_units(multiply_units(
            unit("h", contract=ARITH_CONTRACT, cold_dir=None, name="pe26_ph"),
            unit("x", contract=ARITH_CONTRACT, cold_dir=None, name="pe26_px")).value,
            unit("y", contract=ARITH_CONTRACT, cold_dir=None, name="pe26_py")).value
        identity = tuple(range(8))
        transported = coherent_transport(p, identity)
        require(transported.status == "EXACT" and transported.witness["used_cold"] is False,
                "Coherent identity transport of the hot product must not demand a new distinction")
        return {"q_addition_family": doubled.value.family,
                "site_h_value": str(eval_retained(doubled.value.retained, 1)),
                "hot_independent_sum": {
                    "api_fiber": hot_sum.disposition,
                    "api_status": hot_sum.status,
                    "api_counts": "unique_C2_output_summary",
                    "mathematical_independent_full_sums": ["0", "hxy", "2hxy"],
                    "self_add_full_sums": ["0", "2hxy"],
                    "candidate_propagation": "OUTSIDE_THIS_API"},
                "coherent_identity_on_product": transported.status,
                "boolean_source_fiber_of_zero_c2": "MANY(2) via boolean_fiber_for_compact, not this add result"}
    case("PE26_OUTPUT_CLASS_AND_FIBERS", output_class_and_fibers)
...
def Envelope_from(env, **kwargs):
    ...
    data = {
        ...
        "recipe": env.recipe, "degree_bound": env.degree_bound,
    }
    data.update(kwargs)
