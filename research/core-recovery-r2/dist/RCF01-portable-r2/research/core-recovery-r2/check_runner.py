...
def expected_ok(row: dict) -> tuple[bool, str]:
    ...
    if case == "starter_failure":
        reasons = " ".join(item.get("reason", "") for item in row.get("verdict_failures") or [])
        if "starter" not in reasons:
    ...
    if case == "negative_control_unrelated_crash":
        reasons = " ".join(item.get("reason", "") for item in row.get("verdict_failures") or [])
        if "coverage-rejection" in reasons or "missing output" in reasons or "malformed" in reasons:
            return True, "crash distinguished from coverage rejection"
        if any(name in reasons for name in ("omit", "duplicate", "unexpected")):
