...
**RCF01-envelope-1-r1 · successor of frozen RCF01-envelope-1.**
Original identities 1–5 are unchanged. Identity 6 is the review-repair
composition clarification. Evidence grade: written proof, separate from
finite tests. This file does not rewrite the frozen original
...
## Identity 6 — composed product, recipes, and which fiber a status names

**Carrier.** Same three-cube as Identity 1. Coordinate units `h,x,y` are
the indicator laws of the three bits. Pointwise multiplication is over `Q`.

**Hot summary.** `p = (h*x)*y = hxy`. Every B2 value of `p` is `0`, so
`C2(p)` is the zero 7-tuple. That is the unique retained output summary of this
chain, not an arithmetic error. The omitted coefficient is `1`.

**Fixed-receiver `h`-flip.** `p(1-h,x,y) = xy - hxy`. The B2 readout at
`(0,1,1)` (mask 6) is `1`. The actual zero law remains `0` at that site.

**Class escape.** `h,x,y` have degree 1. `h*x` has degree 2. `(h*x)*y` has
degree 3. Affine-class (`degree <= 1`) reuse of `C2` under `flip_h` is not
inherited after the first multiplication.

**Recipes versus hot arithmetic.** Compact `+`/`*` use only B2 values (Identity
1). A composition recipe, when both operands have verified backing, reconstructs
the **composed** full table by reopening those operands and applying the same
pointwise operation. It is not an operand's pre-product table and not a silent
zero fill of the omitted site. Recipe construction is a dependency artifact, not
hot C2 arithmetic.

**Object versus output fibers.** On Boolean-valued laws, each C2 class has
`MANY(2)` source tables. Adding independently chosen members of the zero-C2
class `{0, hxy}` yields full sums `{0, hxy, 2hxy}` with the same C2. Adding a
single chosen source to itself yields `{0, 2hxy}`. The envelope `add_units`
result `ONE/EXACT` names the unique C2 **output summary**. This API does not
propagate the full candidate family. Boolean-valued functions are not closed
under addition over `Q`.

**Finite-test bound.** PE23–PE26 on this successor. Not a theorem about
arbitrary Python programs or off-cube algebra.

**Hostile.** Padding omitted with `0` after the hot product reconstructs the
zero law, not `hxy`. Using an operand's flipped table is a different question.
