#!/usr/bin/env python3
"""Operation-aware Prestige envelope for RCF01.
...
ENVELOPE_VERSION = "RCF01-envelope-1"
...
def add_units(a: Envelope, b: Envelope) -> Result:
    ...
    out = Envelope(
        family=a.family, visible_unit_label=a.visible_unit_label,
    ...
    return Result("ONE", "EXACT", value=out)
...
def _reopen_cube_table(env: Envelope) -> Result:
    if env.cold_route is None or env.cold_sha256 is None:
        return Result("MANY", "REOPEN_REQUIRED",
                      witness={"omitted_site": OMITTED, "reason": "no cold artifact"})
    try:
        payload = read_cold_artifact(
            Path(env.cold_route), env.cold_sha256,
            {"carrier_id": env.carrier_id, "frame_id": env.frame_id,
             "version": env.version, "family": env.family},
        )
    ...
    return Result("ONE", "EXACT", value=table)
...
def apply_input_map(env: Envelope, map_name: str) -> Result:
    ...
    if map_closes_b2(mapping):
        ...
        out = Envelope(
            ...
            cold_route=env.cold_route, cold_sha256=env.cold_sha256,
            history=env.history,
        )
...
def apply_fixed_receiver_map(env: Envelope, mapping: Sequence[int]) -> Result:
    ...
    if map_closes_b2(mapping):
        ...
        out = Envelope(
            ...
            retained=retained, contract=env.contract,
            carrier_id=env.carrier_id, frame_id=env.frame_id, version=env.version,
            cold_route=env.cold_route, cold_sha256=env.cold_sha256, history=env.history,
        )
        return Result("ONE", "EXACT", value=out, witness={"closed_on_B2": True})
    reopened = _reopen_cube_table(env)
    if reopened.status != "EXACT":
