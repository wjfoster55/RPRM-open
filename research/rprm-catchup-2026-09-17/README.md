...
| [`notes/rho-ehrenfest.md`](notes/rho-ehrenfest.md) | `rho` computed on a partition nobody invented: the textbook Ehrenfest lumping. The domain condition discards ~95% of operations by `d=6`, and shape alone moves the monoid order by 10¹⁸ at fixed state and block count. Also contains a conjecture this project raised and then **refuted** the same night — `rho` is *not* monotone along dominance; minimal counterexample `(9,4,4) → (9,5,3)` at `n=17` |
