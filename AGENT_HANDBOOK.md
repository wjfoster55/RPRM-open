...
For the manuscript's bounded-side Fermat theorem, a,b,c are positive integers
and n is an integer greater than two:
...
The repository also implements the written fixed-gap decision. For supplied
exact integers `n>=2, s>0, d>0`, seek positive integer a with
`D(a)=a^n+(a+s)^n-(a+s+d)^n=0`. The normalized residual `D(a)/a^n` strictly
increases for positive a. The derived bound `M=2*n*(s+d)` has
`D(0)<0<D(M)`. Exact bisection yields adjacent L,U with
